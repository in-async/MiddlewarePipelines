...
namespace Inasync.OnionFunc.Benchmark {
    ...
    public class OnionFuncBenchmark {
        ...
        public void Setup() {
            ...
            for (var i = 0; i < Layers; i++) {
                _pipelineByNext = _pipelineByNext.Layer(next => context => next(context));
            }

            _pipelineByContextAndNext = _handler;
            for (var i = 0; i < Layers; i++) {
                _pipelineByContextAndNext = _pipelineByContextAndNext.Layer((context, next) => next(context));
            }
        }
        ...
        public Func<object, Task> Build_ByNext() {
            ...
            for (var i = 0; i < Layers; i++) {
                pipeline = pipeline.Layer(next => context => next(context));
            }
        ...
        public Func<object, Task> Build_ByContextAndNext() {
            ...
            for (var i = 0; i < Layers; i++) {
                pipeline = pipeline.Layer((context, next) => next(context));
            }
    ...
    internal static class OnionFuncExtensions {
        ...
        public static Func<T, TResult> Layer<T, TResult>(this Func<T, TResult> onionFunc, Func<T, Func<T, TResult>, TResult> middleware) {
